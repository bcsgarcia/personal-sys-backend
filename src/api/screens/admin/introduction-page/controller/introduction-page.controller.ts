...
export class IntroductionPageController {
  constructor(private readonly introductionPageService: IntroductionPageService) {}
  ...
  getCompanyInformation(@Req() request: Request) {
    try {
      ...
      return this.introductionPageService.getCompanyInformation(user.clientIdCompany);
    } catch (error) {
      throw error;
    }
  }
  ...
  createBeforeAndAfterImage(@Body() createItemDto: CreateItemDto, @Req() request: Request) {
    const user = new AccessTokenModel(request['user']);
  ...
  deleteBeforeAndAfterImage(
    ...
    return this.introductionPageService.deleteBeforeAndAfterImage(deleteBeforeAndAfterImageDto);
  }

  @Post('/testimony')
  createTestimony(@Body() createItemDto: CreateItemDto, @Req() request: Request) {
    const user = new AccessTokenModel(request['user']);

    return this.introductionPageService.createTestimony(createItemDto.testimonyToInsert, user.clientIdCompany);
  }

  @Delete('/testimony')
  deleteTestimony(@Body() deleteTestimonyDto: DeleteTestimonyDto, @Req() request: Request) {
    const user = new AccessTokenModel(request['user']);
