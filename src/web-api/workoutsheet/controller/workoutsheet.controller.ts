...
import { ApiBadRequestResponse, ApiBody, ApiHeader, ApiOperation, ApiResponse, ApiTags } from '@nestjs/swagger';
...
export class WorkoutsheetController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: 'The default workout sheet has been successfully created.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad Request.' })
  @ApiResponse({ status: HttpStatus.CONFLICT, description: 'A duplicate key was found in the database.' })
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: 'The default workout sheet has been successfully updated.' })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad Request.' })
  ...
  @ApiOperation({
    ...
    description: 'This endpoint retrieves an array of workoutsheetDefault to be displayed web platform workoutSheetDefault screen.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiBadRequestResponse({ description: 'Bad request, unable to retrieve workoutSheetDefault.' })
  findAllWorkoutSheetDefaultByIdCompany(
    @Req() request: Request
  ) {
    try {
      ...
      return this.workoutsheetService.getAllWorkoutSheetDefaultByIdCompany(idCompany);
  ...
  @ApiBadRequestResponse({ description: 'Bad request, unable to deactivate the workout sheet default.' })
  delete(
    @Param('id') id: string,
    @Req() request: Request
  ) {
